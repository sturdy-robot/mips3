...
impl Cpu {
    pub fn execute_instruction(&mut self, instruction: Instruction) {
        match instruction {
            Instruction::RType { rs, rt, rd, shamt, func } => {
                match func {
                    0x00 => self.instr_sll(rt, rd, shamt),
    ...
    pub fn instr_slt(&mut self, rs: usize, rt: usize, rd: usize) {
        let result = if self.read_register(rs) < self.read_register(rt) { 1 } else { 0 };
        self.write_register(rd, result);
    }
    ...
    pub fn instr_slti(&mut self, rs: usize, rt: usize, immediate: u32) {
        let result = if self.read_register(rs) < immediate { 1 } else { 0 };
        self.write_register(rt, result);
    }
    ...
    #[inline(always)]
    pub fn instr_sll(&mut self, rt: usize, rd: usize, shamt: u32) {
        let result = self.read_register(rt) << shamt;
        self.write_register(rd, result);
    }
